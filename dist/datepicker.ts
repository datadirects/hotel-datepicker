...
const HotelDatepicker = (function (fecha) {
    'use strict';
    ...
    class HotelDatepicker {
      constructor(input, options) {
        ...
        this.showWeekDaysOnTop = opts.showWeekDaysOnTop === undefined ? false : opts.showWeekDaysOnTop;
        this.hideMoveButtons = opts.hideMoveButtons === undefined ? false : opts.hideMoveButtons;
        ...
        this.i18n = {
          ...
          "info-range-equal": "Please select a date range of %d days",
      ...
      getWeekDayNamesWithFlex() {
        let week = "";

        // Start from monday if we passed that option
        if (this.startOfWeek === "monday") {
          for (let i = 0; i < 7; i++) {
            week += '<div class="' + this.className + '__header-week-day">' + this.lang("day-names-short")[(1 + i) % 7] + "</div>";
          }
        ...
        for (let i = 0; i < 7; i++) {
          week += '<div class="' + this.className + '__header-week-day">' + this.lang("day-names-short")[i] + "</div>";
        }
      ...
      init() {
        ...
        // Show months
        let monthDate = new Date(defaultTime);
        for (let i = 1; i <= 6; i++) {
          this.showMonth(new Date(monthDate), i);
      ...
      createDatepickerDomString() {
        ...
        if (this.showWeekDaysOnTop) {
          html += '<div class="' + this.className + '__header-week-days">' + this.getWeekDayNamesWithFlex() + '</div>';
        }

        // Months section
        html += '<div class="' + this.className + '__months aniltest" role="application" aria-roledescription="datepicker" aria-label="' + this.i18n["aria-application"] + '">';

        // Print single months
        for (let i = 1; i <= 6; i++) {
          html += `
           <table role="presentation" id="${this.getMonthTableId(i)}" class="${this.className}__month ${this.className}__month--month${i}">
             <thead>
               <tr class="${this.className}__month-caption">
                 <th>
                   <span
                     role="button"
                     tabindex="0"
                     aria-label="${this.i18n["aria-prev-month"]}"
                     class="${this.className}__month-button ${this.className}__month-button--prev"
                     month="${i}"
                     ${this.hideMoveButtons ? 'style="visibility:hidden;pointer-events:none" aria-hidden="true" tabindex="-1"' : ''}
                   >&lt;</span>
                 </th>
                 <th colspan="5" class="${this.className}__month-name"></th>
                 <th>
                   <span
                     role="button"
                     tabindex="0"
                     aria-label="${this.i18n["aria-next-month"]}"
                     class="${this.className}__month-button ${this.className}__month-button--next"
                     month="${i}"
                     ${this.hideMoveButtons ? 'style="visibility:hidden;pointer-events:none" aria-hidden="true" tabindex="-1"' : ''}
                   >&gt;</span>
                 </th>
               </tr>
               <tr class="${this.className}__week-days" aria-hidden="true" role="presentation">
